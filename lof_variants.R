...
{
gene_info = fread("gene_info.csv")
gene_info<-gene_info[, .(start=min(Start),stop=max(Stop),LGn=LGn[1],gene.name=gene.name[1]), by=GeneID]
...
DP4_calculation<-function(DP4){
  ...
  return(list(dp_alt_f,dp_alt_r))
}
lof_all[, c("DP_ALT_f","DP_ALT_r") := DP4_calculation(DP4) ]
lof_all[,ALF:=(DP_ALT_f+DP_ALT_r)/DP]

# filter low quality records, cannot use IDV==1,as it drops NA as well
lof_all_filtered<-lof_all[!(QUAL<20 | DP<20 | IDV %in% c(1:3) | MQ<30 | DP_ALT_f<3 |DP_ALT_r<3),]

# for the ones that are within a couple of bps, only keep one with the largest alternative allele depth
...
lof_all_filtered[ Pos-Pos_lag<20 | Pos_lead-Pos<20, Grouping:= 0L]
lof_all_filtered<-lof_all_filtered[lof_all_filtered[,.I[which.max(DP_ALT_f+DP_ALT_r)],by=rleid(Grouping)]$V1]
...
temp<-lof_all_filtered[frameshift==T,.(No_frameshift=sum(frameshift==T),Pos=Pos,conseq=conseq,ALF=ALF,frameshift=frameshift,REF=REF,ALT=ALT),by=c("lake","gene")][No_frameshift>1]
...
lof_all_filtered<-lof_all_filtered[!frame_restored_loci,on=.(lake,gene,Pos)]
rm(temp)
...
lof_all_filtered[,(22:29):=lapply(.SD, max),.SDcols=22:29, by=list(LGn, Pos)]
lof_all_filtered_bylocus<-unique(lof_all_filtered, by=c("LGn","Pos"))
lof_all_filtered_bylocus<-lof_all_filtered_bylocus[,c("LGn","Pos","gene","REF","ALT","conseq","DP","IDV","DP_ALT_f","DP_ALT_r",
                                                      "frameshift","stop_gained","stop_lost","start_lost","splice",
...
temp[,(16:23):=lapply(.SD, max),.SDcols=16:23, by=list(LGn, gene)]
lof_all_filtered_bygene<-unique(temp, by=c("LGn","gene"))
...
lof_all_filtered_bygene[, col_min:= colnames(.SD)[max.col(-.SD, ties.method = "first")],.SDcols=16:23]
lof_all_filtered_bygene[, col_max:= colnames(.SD)[max.col(.SD, ties.method = "first")],.SDcols=16:23]
lof_all_filtered_bygene[, No_pres_pop:=rowSums((sapply(.SD, function(x) x>0))),.SDcols=16:23]
# find gene ID for all genes, first based on gene name, then based on position
lof_all_filtered_bygene[,gene_ID:= ifelse(grepl("^ENS",gene), gene,NA )]
lof_all_filtered_bygene[is.na(gene_ID),gene_name:= gene]
lof_all_filtered_bygene[is.na(gene_ID),gene_ID:= gene_info[match(gene_name,gene_info[,gene.name]),GeneID]]
temp<-gene_info[lof_all_filtered_bygene[is.na(gene_ID)],on=c("LGn", "start<=Pos", "stop>=Pos"),mult="first"]
lof_all_filtered_bygene[is.na(gene_ID),gene_ID:= temp[match(gene_name,temp[,gene]),GeneID]]

plot(table(lof_all_filtered_bygene[No_pres_pop==1, col_max]))  # rob has the most pop specific lof alleles
lof_all_filtered_bygene[No_pres_pop==8,.N] # 1741 lof genes are shared by all pops
lof_all_filtered_bygene[No_pres_pop==7 & col_min=="ALF_say",gene] # 124 lof genes are only present in freshwater lakes
lof_all_filtered_bygene[No_pres_pop==1 & col_max=="ALF_say",gene] # 165 lof genes are only present in say



rob0.9_gos0_lof<-lof_all_filtered_bygene[ALF_gos==0 & ALF_rob>0.9,]
gos0.9_rob0_lof<-lof_all_filtered_bygene[ALF_rob==0 & ALF_gos>0.9,]

fwrite(rob0.9_gos0_lof,"./Results/rob0.9_gos0_lof.csv")
fwrite(gos0.9_rob0_lof,"./Results/gos0.9_rob0_lof.csv")
...
rob0.9_gos0_lof_expression<-expression_rob_gos_population[rob0.9_gos0_lof,on=c("V1"="gene_ID")][!is.na(baseMean),]
gos0.9_rob0_lof_expression<-expression_rob_gos_population[gos0.9_rob0_lof,on=c("V1"="gene_ID")][!is.na(baseMean),]
    
t.test(rob0.9_gos0_lof_expression[,stat],gos0.9_rob0_lof_expression[,stat])
t.test(rob0.9_gos0_lof_expression[,log2FoldChange],gos0.9_rob0_lof_expression[,log2FoldChange])
boxplot(rob0.9_gos0_lof_expression[pvalue<0.05,log2FoldChange],gos0.9_rob0_lof_expression[pvalue<0.05,log2FoldChange])
boxplot(rob0.9_gos0_lof_expression[,log2FoldChange],gos0.9_rob0_lof_expression[,log2FoldChange])
...
length_rob<-dim(rob0.9_gos0_lof_expression)[1]
length_gos<-dim(gos0.9_rob0_lof_expression)[1]
